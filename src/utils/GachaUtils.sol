...
library GachaUtils {
  ...
  function addItems(uint256 gachaId, uint256[] memory itemIds) public {
  ...
  function addItem(uint256 gachaId, uint256 itemId) public {
  ...
  function removeItems(uint256 gachaId, uint256[] memory itemIds) public {
    for (uint256 i = 0; i < itemIds.length; i++) {
  ...
  function removeItem(uint256 gachaId, uint256 itemId) public {
  ...
  function hasItem(uint256 gachaId, uint256 itemId) public view returns (bool) {
    uint256 index = GachaItemIndex.get(gachaId, itemId);
    return index != 0;
  }
}
