...
import { CharStorage, StrToolIndex } from "@codegen/index.sol";
...
library StorageToolUtils {
  ...
  function _addTool(uint256 characterId, uint256 cityId, uint256 toolId) private {
    ...
    StrToolIndex.set(characterId, cityId, toolId, index);
  }
  ...
  function _removeTool(uint256 characterId, uint256 cityId, uint256 toolId) private {
    uint256 index = StrToolIndex.get(characterId, cityId, toolId);
    ...
    if (valueIndex != lastIndex) {
      ...
      StrToolIndex.set(characterId, cityId, lastValue, index);
    }
    CharStorage.popToolIds(characterId, cityId);
    StrToolIndex.deleteRecord(characterId, cityId, toolId);
  }
