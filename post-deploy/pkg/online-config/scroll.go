...
func getListScrollUpdate(sheetName string, dataConfig *common.DataConfig) ([]common.Item, []common.ItemRecipe, error) {
	...
	for i := range rawData {
		...
		if strings.EqualFold(record[0], "id") { // header
			...
			numTargetIndex = findIndex(record, "num_target")
			isBuffIndex = findIndex(record, "is_buff")
			selfCastOnlyIndex = findIndex(record, "self_cast_only")
			atkPercentIndex = findIndex(record, "atk_percent")
			defPercentIndex = findIndex(record, "def_percent")
			agiPercentIndex = findIndex(record, "agi_percent")
			msIndex = findIndex(record, "ms")
			spIndex = findIndex(record, "sp")
			farmingPerkAmpIndex = findIndex(record, "farming_perk_amp")
			pveExpAmpIndex = findIndex(record, "pve_exp_amp")
			// pvePerkAmpIndex = findIndex(record, "pvePerkAmp")
			dmgIndex = findIndex(record, "dmg")
			isAbsDmgIndex = findIndex(record, "is_abs_dmg")
		...
		item.BuffInfo = &common.BuffItemInfo{
			Type:         scrollType,
		...
		scrollRecipe := common.ItemRecipe{
			ItemId:      id,
